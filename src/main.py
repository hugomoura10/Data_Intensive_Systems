...
def get_traces(user_id,df):
    if "features" in df.columns:
        ...
        if result:
            ...
            return None
    if "to" in df.columns:
        df_filtered = df.filter(df.type.isin(['Req']))
        df_grouped = df_filtered.groupBy("user_id").agg(concat_ws("",collect_list("to")).alias("features"))
        result = df_grouped.filter(col("user_id") == user_id).select("features").collect()
        if result:
            return result[0]["features"]
        else:
            return None
...
def experiment(data,k_value,threshold=0,approach =1):
    ...
    print(f"Number of unique processes after merging them with {threshold} threshold using {k_value}-shingles: {len(new_process_dictionary)}")
...
def performance_eval(func1,func2,data,vals):
    results = get_performance(func1,func2,data, vals)
